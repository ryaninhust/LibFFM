...
"-m <model>: set path to the existing model\n"
...
struct Option
{
    ...
    string tr_path, va_path, model_path, warm_model_path;
    ffm_parameter param;
    ffm_int nr_folds;
    bool do_cv, on_disk;
};
...
int train(Option opt)
{
    ...
    ffm_model *raw_model = nullptr;
    if (!opt.warm_model_path.empty())
    {
        raw_model = ffm_load_model(opt.warm_model_path.c_str());
        if (raw_model == nullptr) {
            ffm_destroy_problem(&tr);
            cerr << "cannot load " << opt.warm_model_path << endl << flush;
            return 1;
        }
    }
    ...
    if(opt.do_cv)
    {
        ffm_cross_validation(tr, opt.nr_folds, opt.param, raw_model);
    }
    else
    {
        ffm_model *model = ffm_train_with_validation(tr, va, opt.param, raw_model);


        status = ffm_save_model(model, opt.model_path.c_str(), opt.param.k);
...
int train_on_disk(Option opt)
{
    ...
    ffm_model *model = ffm_train_with_validation_on_disk(tr_bin_path.c_str(), va_bin_path.c_str(), opt.param, raw_model);

    ffm_int status = ffm_save_model(model, opt.model_path.c_str(), opt.param.k);
    if(status != 0)
    {
        ffm_destroy_model(&model);
